view: student_demographics {
   derived_table: {
     sql: select ss.district_key, ss.location_key, ss.school_year, count(*) enrollment, sum(case when ss.student_gender_code='F' then 1 else 0 end) female_students,
           ...
           where ss.student_snapshot_date=(select max(ss2.student_snapshot_date) from stars.student_snapshot ss2 where ss2.school_year=ss.school_year and month(ss2.student_snapshot_date) in ('6', '10', '12', '3') and day(ss2.student_snapshot_date) in ('01', '15'))
